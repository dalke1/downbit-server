...
public class CosUtil {
    ...
    // 生成签名的过期时间
    Date expiration = new Date(System.currentTimeMillis() + TimeUnit.DAYS.toMillis(1) + TimeUnit.HOURS.toMillis(1));
...
//            throw new EmptyFileNameException("头像文件名为空");
        ...
        String url = getUrl(key, expiration, HttpMethodName.GET, signature, "image/jpeg");
    ...
    public String getVideoUrl(String username, String fileName) {
        ...
        String url = getUrl(key, expiration, HttpMethodName.GET, signature, "video/mp4");
    ...
    public String getUploadAvatarUrl(String username, String filename) {
        String key = "avatar/" + username + "/" + filename;
        return getUrl(key, uploadExpiration, HttpMethodName.PUT, "avatar:" + UUID.randomUUID(), "image/jpeg");
    }

    public String getUploadCoverUrl(String username, String filename) {
        String key = "cover/" + username + "/" + filename;
        return getUrl(key, uploadExpiration, HttpMethodName.PUT, "cover:" + UUID.randomUUID(), "image/jpeg");
    }

    public String getUploadVideoUrl(String username, String filename) {
        String key = "video/" + username + "/" + filename;
        return getUrl(key, uploadExpiration, HttpMethodName.PUT, "video:" + UUID.randomUUID(), "video/mp4");
    }

    public String getUrl(String key, Date expiration, HttpMethodName method, String signature, String contentType) {
        ...
        if (method == HttpMethodName.GET) {
            ...
            // 设置响应头为需要缓存
            responseHeaders.setCacheControl("max-age=" + TimeUnit.HOURS.toMillis(25));
